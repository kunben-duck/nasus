...
class AgentGoalProposal:
    ...
    planning_usage: ModelUsage = field(default_factory=ModelUsage)
    max_steps: int = 50
    max_model_calls: int = 32
    max_thinking_tokens: int = 500_000
...
def agent_goal_proposal_from_payload(payload: dict[str, Any]) -> AgentGoalProposal:
    ...
    return AgentGoalProposal(
        ...
        planning_usage=ModelUsage(
            model_calls=int(usage_payload.get("model_calls") or 0),
            input_tokens=int(usage_payload.get("input_tokens") or 0),
            output_tokens=int(usage_payload.get("output_tokens") or 0),
            usage_source=usage_payload.get("usage_source", "none"),
        ),
        max_steps=int(payload.get("max_steps") or 50),
        max_model_calls=int(payload.get("max_model_calls") or 32),
        max_thinking_tokens=int(payload.get("max_thinking_tokens") or 500_000),
        max_runtime_seconds=int(payload.get("max_runtime_seconds") or 1_800),
        max_no_progress_observations=int(
            payload.get("max_no_progress_observations") or 3
        ),
    )
