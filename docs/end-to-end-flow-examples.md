...
| 来源 | 目标 | 通信方式 | 首发默认值 |
| --- | --- | --- | --- |
| Frontend / Desktop UI | `api/orchestrator` | REST + SSE | 固定 |
...
| `workflow-service` | `sync-service` | REST / internal event | 固定 |
| `worker-runtime` | `llm-gateway` | 内部 SDK / service call | 固定 |
| `workflow-service` | PostgreSQL / MinIO | ORM / object storage client | 固定 |
| `sync-service` | Desktop Client | REST + SSE + signed upload | 固定 |

规则：

- 不允许前端直接调 worker、runner、sync-service。
...
2. Agent 调用：
   - `automation.generate`
   - `run.start`
3. `run.start` 创建 `Run(execution_channel=web_runner|desktop_local)`
...
1. Web 端和 Desktop 端分别对同一 `Task` 产生 `AgentDecision(status=provisional)`
2. `Merge / Score` 检测冲突
