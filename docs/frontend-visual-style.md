...
- 将视觉语义分为三类：`探索`（导航 + 状态），`工作`（Tasks / Runs / Knowledge 中心区），`治理`（Settings + Approval）。每类保持一致的色调与动效节奏。
...
- Home 页面保留 AI Studio 风格的大面积留白：中心是单句 Hero 标题和能力卡片，底部是固定式输入/发起面板。Tasks/Knowledge/Runs 在同一壳层上切换为“内容驱动”视图。
...
## 5. 主工作区（Home / Tasks / Knowledge / Runs）
- 背景采用 #0f1218，区域卡片深度通过多层阴影（`box-shadow: 0 15px 30px rgba(0,0,0,0.35)`）体现。
- 卡片采用圆角 18px，内边距 24px，卡片之间垂直间距 16px，卡片顶部可插入水平步骤条或状态条。
- Home 的 Hero 标题可放大到 48px-56px；常规页面标题为 24px-28px，次级标题 18px semibold。
- Home 聚合快速启动卡片，采用 2 x 3 或 3 x 2 能力卡片网格，并在底部保留固定式输入框。
- Tasks 使用“任务头部 + 任务画布 + 右侧策略面板 + 底部主操作输入框”的结构；输入框用于发起补充分析、追加材料、请求重新生成，而不是单纯聊天。
- Knowledge 使用“图谱/列表 + 对象详情 + 证据细节”的双层结构；Runs 使用“运行列表/时间线 + 运行详情 + 右侧过滤/放行”结构。
...
- Home 页面中输入框承担“开始一个任务”；Tasks 页面中承担“补充上下文并请求下一步动作”；Runs 页面中承担“请求归因/重跑/提交放行”的快捷动作。
...
- `< 768px`：左侧导航改为 overlay drawer，右侧策略面板转为 bottom sheet；Home 能力卡片改为单列，Tasks/Runs 的时间线与详情采用上下堆叠。
...
以上规范可直接供前端设计与开发参考，用于生成 Home、Tasks、Knowledge、Runs、Settings 以及审批与知识图谱等子体验。
