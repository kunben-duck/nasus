...
首版必须预留 hybrid retrieval 投影：

- 关键词检索：模块名、US 编号、接口路径、测试用例名
- 语义检索：相似 US、相似测试场景、相似失败模式
- 元数据过滤：project/version/baseline/type/status/freshness

物理实现可以从 PostgreSQL FTS / pgvector 起步，也可以直接接 OpenSearch / Elasticsearch。无论实现方式如何，上层只能通过 UCE 查询接口访问，不能直接绑定具体引擎。

### 11.1.5 Graph Projection
...
- Search / Vector 从 V1 起作为长期架构的一部分预留接口；具体物理实现可按规模选择 PostgreSQL FTS/pgvector 或 OpenSearch。
