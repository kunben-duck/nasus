...
- 工具目录以 [docs/tool-catalog-v1.md](/Users/uben/project/project/Nasus/docs/tool-catalog-v1.md) 为准。
- 工具调用协议以 [docs/backend-runtime-and-tool-protocol.md](/Users/uben/project/project/Nasus/docs/backend-runtime-and-tool-protocol.md) 为准。
...
在 [backend-api-and-events.md](/Users/uben/project/project/Nasus/docs/backend-api-and-events.md) 的公开事件类型基础上，新增以下 Agent Loop 事件：
...
Agent Loop 完全继承 [backend-execution-sync-governance.md](/Users/uben/project/project/Nasus/docs/backend-execution-sync-governance.md) 中定义的 Gate 规则：
...
- 即使在 `full_auto` 模式下，RBAC、PolicySnapshot、CapabilityGrant 仍全部生效。
...
| 文档 | 关系 |
| --- | --- |
| `backend-system-design.md` | Agent Loop Runtime 是后端新增的一个逻辑层，位于 `Conversation Layer` 和 `Tool Contract Layer` 之间 |
| `conversation-orchestrator-design.md` | Orchestrator 新增 `AgentGoalProposal` 输出类型 |
| `backend-runtime-and-tool-protocol.md` | LangGraph 的角色从"工具内部编排"扩展为"Agent Loop + 工具内部"双层编排 |
| `backend-domain-model.md` | 新增 `AgentGoal` 和 `AgentStep` 两个领域对象 |
| `backend-api-and-events.md` | 新增 Agent Loop 相关 API 和 SSE 事件 |
| `backend-execution-sync-governance.md` | Gate 规则完全继承，不做任何绕过 |
| `tool-catalog-v1.md` | Tool Catalog 不变，Agent Loop 是 Tool 的消费者而不是替代者 |
| `llm-provider-and-runtime-design.md` | 新增 `agent_loop_planner` Prompt 模板和对应的 token 预算维度 |
| `auth-and-access-design.md` | Agent 权限边界不变，`effective_actor` 始终是用户 |
