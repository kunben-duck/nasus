...
- 工具必须先有 `ToolDefinition`，再允许被 Agent、UI 或 Desktop 触发
...
| tool_id | 用途 | 默认 scope | 风险 |
| --- | --- | --- | --- |
| `us.task.start` | 启动某个 US 的质量闭环 | `either` | `low` |
...
| `run.start` | 触发执行 | `either` | `medium` |
| `run.progress.get` | 查询执行进度与证据摘要 | `either` | `low` |
| `failure.analyze` | 分析失败原因 | `either` | `low` |
| `healing.propose` | 生成修复建议 | `either` | `medium` |
...
### 3.6 Device / Edge Tools

| tool_id | 用途 | 默认 scope | 风险 |
| --- | --- | --- | --- |
| `device.register` | 注册 Desktop 设备 | `edge` | `medium` |
| `device.capabilities.request` | 申请本地能力授权 | `edge` | `high` |
| `device.local.execute` | 执行本地受控动作 | `edge` | `high` |
| `sync.replay` | 回放离线补传 | `edge` | `medium` |

### 3.7 Query / Insight Tools
