...
| 环境 | 默认组成 | 目标 |
| --- | --- | --- |
| `local` | `api/orchestrator` `workflow-service` `worker-runtime` `runner` `sync-service` `PostgreSQL` `MinIO` `OpenGrok` | 单开发者联调 |
| `dev` | 本地同构 + 共享索引 + 测试设备 | 多人共享开发 |
| `staging` | 完整中心端 + 桌面同步链路 + 审计与策略 | 预发布验证 |
| `prod` | 私有化部署 + 隔离执行网络 + 设备分级管理 + 全量监控备份 | 正式运行 |
...
- device session -> local run -> sync replay
...
- sandbox escape attempt / unauthorized local path write

### 6.6 验收测试

- 用户在主会话发起查询和写动作
- UI 按钮与主会话触发同一动作时产生同类 `ToolInvocation`
- 双端候选结果冲突进入 `pending_merge`
- 高风险动作进入确认/审批闸口
