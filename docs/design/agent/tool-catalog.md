...
| `system_image.sources.register` | 注册系统画像一等 source：代码、历史 US、历史测试资产 | `central` | `low` |
| `system_image.sources.ingest` | 摄入并索引系统画像 source | `central` | `medium` |
| `system_image.context.materialize` | 物化 ContextObject、关系和质量指标 | `central` | `medium` |
| `system_image.baseline.initialize` | 初始化 Historical / Official Baseline | `central` | `high` |
...
| `version.risk.initialize` | 初始化版本风险视图和 US 初始风险 | `central` | `medium` |
...
- `version.import.us` 是历史 alias，必须规范化为 `version.inputs.import`。
- `merge.resolve` 是历史 alias，必须规范化为 `resolution.merge`。
- `query.run.status` 是历史 alias，必须规范化为 `run.progress.get`。
