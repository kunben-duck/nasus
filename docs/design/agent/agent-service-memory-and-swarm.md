...
| `Agent Goal Plan Compiler` | 将高层目标编译为可执行 `ToolPlanStep[]`，可根据系统画像、工具目录和当前领域状态补全或跳过步骤 |
...
- `Conversation Orchestrator` 的 LLM Planner、普通 LLM 回答、Agent Loop THINK 阶段都必须消费同一个 `AgentMemoryContext` 契约。
- `AgentMemoryContext` 至少包含 `system_prompt`、`context_snapshot`、`history_snapshot`、`recent_turn_count` 和 `checkpoint_count`。
- Planner 不能自行拼接一套与普通回答不同的上下文窗口；工具目录、系统画像、最近消息、checkpoint、候选知识和治理约束都必须由 `Agent Memory Manager` 组装。
